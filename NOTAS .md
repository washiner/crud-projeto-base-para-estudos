...
- [x] `feature/dtos` → DTOs de request e response ✅
...
### Por que usar?
- Protege a entidade do domínio
- Controla exatamente o que entra e o que sai
- Dados sensíveis (CPF, senha) entram pelo Request mas nunca saem pelo Response

### Dois tipos:
```
Request  → o que o front MANDA pro back
Response → o que o back DEVOLVE pro front
```

### Quando usar cada um:
```
GET    → não recebe corpo → devolve Response
POST   → recebe Request  → devolve Response
PUT    → recebe Request  → devolve Response
DELETE → recebe só o ID  → devolve Void
```

### Use `record` para DTOs:
```java
// Imutável, sem boilerplate, getters automáticos
public record ViagemRequest(
    ...
    StatusViagem status,
    String cpf        // entra mas não sai no Response!
) {}

public record ViagemResponse(
    Long id,          // o front precisa saber o id
    String destino,
    String pais,
    LocalDate dataPartida,
    LocalDate dataRetorno,
    StatusViagem status
    // CPF não está aqui — protegido! 🔒
) {}
```

### Atenção: record usa `.campo()` não `.getCampo()`
```java
request.destino()     // ✅ correto no record
request.getDestino()  // ❌ errado no record
```

### Fluxo de conversão no Service:
```
Request → cria ViagemModel → seta campos → salva no banco → monta Response → retorna
```

```java
// Converter lista de Model pra Response
...
### Stream — analogia da fábrica:
```
findAll()  → pega todas as peças do estoque
stream()   → coloca na esteira
map()      → cada peça vira outra coisa
toList()   → coloca tudo numa caixa e entrega
```

### Próximo passo: MapStruct
No mercado usam MapStruct pra converter automaticamente — sem setar campo por campo:
```java
ViagemModel model = mapper.toModel(request);
ViagemResponse response = mapper.toResponse(model);
```

---

## 🗄️ SQL — Comandos essenciais
...
-- Inserir
INSERT INTO viagem (destino, pais, data_partida, data_retorno, status, cpf)
VALUES ('Paris', 'França', '2026-10-01', '2026-10-15', 'PLANEJADA', '123.456.789-00');

-- Atualizar
UPDATE viagem SET destino = 'Roma' WHERE id = 1;

-- Deletar
DELETE FROM viagem WHERE id = 1;
```

### ⚠️ Regra de ouro do WHERE:
**Antes de UPDATE ou DELETE, pergunta: quero mexer em UM ou em TODOS?**
- Um → precisa do WHERE
- Todos → sem WHERE (cuidado!)

```sql
UPDATE viagem SET status = 'CONCLUIDA';   -- atualiza TODOS! 😱
DELETE FROM viagem;                        -- deleta TODOS! 😱
```

### Estrutura de tabelas:
```sql
-- Adicionar coluna nova
ALTER TABLE viagem ADD COLUMN cpf VARCHAR(255) NOT NULL DEFAULT 'não informado';

-- Remover coluna
ALTER TABLE viagem DROP COLUMN cpf;

-- Limpar todos os dados mas manter a tabela
...
### Por que usar Flyway no mercado?
O `ddl-auto=update` cria/atualiza tabelas automaticamente mas não versiona as mudanças.
Com Flyway cada mudança vira um arquivo versionado:
```
V1__create_table_viagem.sql
V2__add_column_cpf.sql
V3__add_column_ano.sql
```
Toda equipe fica sincronizada com o banco. Em produção é obrigatório.
